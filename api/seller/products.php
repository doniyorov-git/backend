...
if ($_SERVER['REQUEST_METHOD'] === 'GET') {
    ...
    if ($isUpdate) {
        ...
        ");
        $stmt->execute([$name, $category, $region, $model, $price, $unit, $imagePath, $id, $sellerId]);
        ...
        $stmt = $pdo->prepare("
            INSERT INTO products (id, seller_id, name, sku, category, region, model, price, unit, image, status)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 'pending')
        ");
